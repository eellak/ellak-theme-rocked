...
if (!function_exists('rocked_setup')) :
    ...
    function rocked_setup() {
        ...
        register_nav_menus(array(
            ...
            'usermenu' => esc_html__('User Menu', 'rocked'),
        ));
...
function rocked_scripts() {
    ...
    if (get_theme_mod('blog_layout') == 'masonry-layout' && (is_home() || is_archive())) {
        wp_enqueue_script('rocked-masonry-init', get_template_directory_uri() . '/js/masonry-init.js', array('jquery-masonry'), '', true);
    }
...
function ellak_diadose_style_rocked() {
    wp_register_style('ellak-diadose-css-rocked', get_template_directory_uri() . '/css/diadose_css.css');
    wp_enqueue_style('ellak-diadose-css-rocked');
    
    wp_enqueue_style('ellak-duckduckgo-search-box', get_template_directory_uri().'/css/duck-duck-style.css');
}
...
function rocked_enqueue_bootstrap() {
    wp_enqueue_script('rocked-bootstrap-js', get_template_directory_uri() . '/js/bootstrap.min.js', array('jquery'), '3.0', true);
...
add_action('tgmpa_register', 'rocked_recommend_plugin');

function rocked_recommend_plugin() {

    $plugins = array(
        array(
            'name' => __('Page Builder by SiteOrigin', 'rocked'),
            'slug' => 'siteorigin-panels',
            'required' => false,
        ),
        array(
            'name' => __('Types - Custom Fields and Custom Post Types Management', 'rocked'),
            'slug' => 'types',
            'required' => false,
        ),
    );
